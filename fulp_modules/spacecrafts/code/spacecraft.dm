...
/obj/vehicle/sealed/spacecraft
	...
	icon = 'fulp_modules/spacecrafts/icons/spacecraft.dmi' //We gotta have a placeholder for testing purposes, heh?
	icon_state = "pod_body"
	...
	var/vehicle_move_delay = 1 // The move delay for spacecrafts
	...
	var/overlays_file = 'fulp_modules/spacecrafts/icons/spacecraft_overlays.dmi'
...
/obj/vehicle/sealed/spacecraft/proc/update_part_values()
	...
	if(engine)
		if(engine.rating < 5)
			vehicle_move_delay = 1.5 - 0.3 * engine.rating
		if(engine.rating == 5)
			...
			for(var/mob/occupant in occupants)
				switch(cellcharge)
					...
					if(0.01 to 0.25)
						...
						occupant.throw_alert("charge", /atom/movable/screen/alert/emptycell)            // Figure a way to make this work later.
...
/obj/vehicle/sealed/spacecraft/spacepod
	...
	icon = 'fulp_modules/spacecrafts/icons/spacecraft.dmi'
	icon_state = "pod_body"

//// Movement ////

/obj/vehicle/sealed/spacecraft/relaymove(mob/living/user, direction)
	if(is_driver(user) && canmove && (!key_type || istype(inserted_key, key_type)))
		vehicle_move(direction)
	return TRUE

/obj/vehicle/sealed/spacecraft/vehicle_move(direction)
	...
	if(pressure <= 30 && pressure > 0.1) //Still some pressure? Tough luck, slow as fuck.
		vehicle_move_delay = 25 * 2/speed_modifier
	if(pressure <= 0.1) //Finally in space like intended? Godspeed brother.
		vehicle_move_delay = regular_move_delay * 2 ** (2 - speed_modifier) // So basically, half-speed (double delay), regular speed (normal delay), full speed (half delay)
	if(!COOLDOWN_FINISHED(src, cooldown_vehicle_move) && canmove)
		return FALSE
...
/datum/action/vehicle/sealed/spacecraft/spacecraft_speed_control
	name = "Change Speed"
	icon_icon = 'fulp_modules/spacecrafts/icons/spacecraft_buttons.dmi'
...
/obj/item/spacecraft_parts
	...
	icon = 'fulp_modules/spacecrafts/icons/spacecraft_parts.dmi'
...
/obj/item/spacecraft_parts/arm
	...
	icon = 'fulp_modules/spacecrafts/icons/spacecraft_overlays.dmi'
